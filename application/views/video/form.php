<?php $this->load->view('admin_header', [
    'title' => isset($video) ? 'Edit Video Section' : 'Create Video Section'
]); ?>

<div class="main-content">
    <h2 style="text-align: center; margin-bottom: 20px;">
        <?= isset($video) ? 'Edit Video Section' : 'Create New Video Section' ?>
    </h2>

    <form action="<?= site_url('admin/save_video' . (isset($video['id']) ? '/' . $video['id'] : '')) ?>" method="post" enctype="multipart/form-data">
                ...
                <?php if (!empty($video['filename'])): ?>
                    ...
                    <?php if ($is_video): ?>
                        <video width="320" height="240" controls style="display:block; margin-bottom: 10px;">
                            <source src="<?= base_url('assets/uploads/' . $video['filename']) ?>" type="video/<?= $ext ?>">
                            Your browser does not support the video tag.
                        </video>
                    <?php else: ?>
                        <img id="previewImage"
                             src="<?= base_url('assets/uploads/' . $video['filename']) ?>"
        ...
        <div style="text-align: center; margin-top: 25px;">
            ...
            <a href="<?= site_url('admin/save_video') ?>" style="margin-left: 15px; padding: 10px 20px; background-color: #6c757d; color: white; text-decoration: none; border-radius: 4px;">Cancel</a>
        </div>
    </form>
</div>
